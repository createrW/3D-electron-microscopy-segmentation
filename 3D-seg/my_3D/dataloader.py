...
def load_volume(dataset, labeled=True, slice_range=None):
    ...
    '''
    if labeled:
        path = os.path.join(dataset, "masks", "*.jpg")
    else:
        path = os.path.join(dataset, "images", "*.jpg")
...
class RandomVolumetricDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, shape=(256, 256, 256), length=165, transform=None):
        self.volumes = []
        self.targets = []
        self.length = length
        self.shape = shape
...
ds = RandomVolumetricDataset([
    "E:\\Learning-Py\\3D_seg\\data"
], length=165, transform=transform)
...
# # Showing the same subvoluzzme, with random rotations
...
# bar = tqdm(enumerate(dl), total=len(ds))
# for step, data in bar:
#     image = data[0].to(device)
#     mask = data[0].to(device)
